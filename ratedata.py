...
class RateData:
    ...
    def __init__(self, trajectories_data: dict, inter_location: list, stop_speed_threshold: float,
        ...
        :param sum_length_outbound_: 如果需要输出反向（outbound）, 需要输入反向的路径长度（m），
               两个选项：
        ...
        # 判断方向
        if sum_length_outbound_ is not None:
            if type(sum_length_outbound_) == list:
                self.sum_length_outbound = sum(sum_length_outbound_)
            elif type(sum_length_outbound_) == float:
                self.sum_length_outbound = sum_length_outbound_
...
if __name__ == "__main__":
    ...
    # from datastore import load_variavle, save_variable
    # from rateperformance import output_average_speed, output_stop_delay
    #
    ...
    #                        336.21773410990767]
    # inbound_trajectorydata = load_variavle('D:\\桌面文件\\第四章\\CSPT\\RateArterialData\\inbound_trajectorydata.pkl')
    # outbound_trajectorydata = load_variavle('D:\\桌面文件\\第四章\\CSPT\\RateArterialData\\outbound_trajectorydata.pkl')
    # RD = RateData(inbound_trajectorydata, inter_location_inbound, 0)
    # inbound_rate_data = RD.output_rate_data()
    # RD = RateData(outbound_trajectorydata, inter_location_outbound, 0, sum_length_outbound)
    # outbound_rate_data = RD.output_rate_data()
    # print(inbound_rate_data)
    # print(outbound_rate_data)
    # inbound_rate_data = save_variable(inbound_rate_data, 'inbound_rate_data',
    #                                   'D:\\桌面文件\\第四章\\CSPT\\RateArterialData')
    # outbound_rate_data = save_variable(outbound_rate_data, 'outbound_rate_data',
    #                                    'D:\\桌面文件\\第四章\\CSPT\\RateArterialData')
    # # output_average_speed(inbound_rate_data, 'inbound', 'D:\\桌面文件\\第四章\\CSPT\\exp\\trajectiesdata')
    # output_stop_delay(inbound_rate_data, 4, 'inbound_delay', 'D:\\桌面文件\\第四章\\CSPT\\exp\\trajectiesdata')
